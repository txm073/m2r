...
class Torus:
  ...
  #     print('periodicities:', self.theta_period, self.phi_period)
  ...
  def __call__(
    ...
    a, b = 2 * torch.pi, 2 * torch.pi
    return (
      (self.R + self.r_torch(theta * a, phi * b) * torch.cos(phi * b)) * torch.cos(theta * a),
      (self.R + self.r_torch(theta * a, phi * b) * torch.cos(phi * b)) * torch.sin(theta * a),
      self.r_torch(theta * a, phi * b) * torch.sin(phi * b)
    )
